...
use core::animation::logic::{animator, smooth, transform, bounds};
...
use core::animation::export::encoding::ExportFormat;
use core::settings::logic::state::Settings;
use crate::features::animation::{process, canvas, controls};
use core::animation::logic::canvas as glow_canvas;
...
pub struct AnimViewer {
    ...
    pub staging_sheet: Option<GuiSpriteSheet>,
    pub held_model: Option<Model>,
    pub held_sheet: Option<GuiSpriteSheet>,
    pub renderer: Arc<Mutex<Option<glow_canvas::GlowRenderer>>>,
...
impl AnimViewer {
    ...
    pub fn show(
        ...
        anim_sheet_sync: &mut GuiSpriteSheet,
        ...
        if valid_index != current_index {
            ...
            if valid_index == IDX_NONE {
                ...
                *anim_sheet_sync = GuiSpriteSheet::default();
            }
        ...
        if !is_stable && !is_loading_new && !is_first_launch && valid_index != IDX_NONE {
            ...
            if let (Some(png_path), Some(cut_path), Some(model_path)) = (resolved_png, resolved_cut, resolved_model) {
                let mut new_sheet = GuiSpriteSheet::default();
                new_sheet.load(png_path, cut_path, target_viewer_id.clone());
        ...
        if is_first_launch && valid_index != IDX_NONE {
            ...
            let mut load_success = false;
            if let (Some(png_path), Some(cut_path), Some(model_path)) = (resolved_png, resolved_cut, resolved_model) {
                 anim_sheet_sync.core.image_data = None;
                 anim_sheet_sync.load(png_path, cut_path, target_viewer_id.clone());
                 if let Some(loaded_model) = Model::load(model_path) {
        ...
        if is_loading_new {
            if let Some(staging_sheet) = &mut self.staging_sheet {
                ...
                let texture_is_ready = staging_sheet.core.sheet_name == target_viewer_id
                                    && !staging_sheet.core.is_loading_active
                                    && staging_sheet.core.image_data.is_some();
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.put(viewport_rect, |ui: &mut egui::Ui| {
                                self.draw_viewport(
                                    ...
                                    primary_id, secondary_id, &secondary_assets, allow_texture_update, settings, drag_guard
                                );
    ...
    fn draw_viewport(
            ...
            if self.pending_initial_center {
                if centering_behavior == 0 { 
                    if !model_data.parts.is_empty() {
                        if let Some((offset, fit_zoom)) = bounds::calculate_initial_view(model_data, self.current_anim.as_ref(), &sheet_data.core, ui.available_size().x, ui.available_size().y, settings.animation.use_tight_bounds) {
                     ...
                     if let (Some(model_data), Some(sheet_data)) = (&self.held_model, &self.held_sheet) {
                        ...
                        let master_bounds = if self.export_state.export_mode == ExportMode::Showcase {
                            bounds::calculate_showcase_bounds(
                                model_data, &sheet_data.core, available_anims,
                            ...
                            if settings.animation.use_tight_bounds {
                                bounds::calculate_tight_bounds(model_data, self.current_anim.as_ref(), &sheet_data.core)
                            } else {
                                bounds::calculate_loose_bounds(model_data, self.current_anim.as_ref(), &sheet_data.core)
                            }
                        };

                        if let Some(calculated_bounds) = master_bounds {
                            self.export_state.region_x = calculated_bounds.min_x;
                            self.export_state.region_y = calculated_bounds.min_y;
        ...
        if settings.animation.export_popup_open && self.export_state.export_mode == ExportMode::Showcase && !self.has_scanned_showcase {
             let attack_animation_opt = available_anims.iter().find(|(index, _)| *index == IDX_ATTACK)
             ...
             let walk_animation_opt = available_anims.iter().find(|(index, _)| *index == IDX_WALK)
                 .and_then(|(_, path)| Animation::load(path));
             ...
             let idle_animation_opt = available_anims.iter().find(|(index, _)| *index == IDX_IDLE)
                 .and_then(|(_, path)| Animation::load(path));
        ...
        if let (Some(model_data), Some(sheet_data)) = (&self.held_model, &self.held_sheet) {
            ...
            let sheet_arc = Arc::new(GuiSpriteSheet {
                texture_handle: sheet_data.texture_handle.clone(),
                core: core::global::formats::imgcut::SpriteSheet {
                    image_data: sheet_data.core.image_data.clone(),
                    cuts_map: sheet_data.core.cuts_map.clone(),
                    is_loading_active: sheet_data.core.is_loading_active,
                    data_receiver: None,
                    sheet_name: sheet_data.core.sheet_name.clone()
                }
            });
        ...
        let controls_hovered = render_controls_overlay(ui, rect_alloc, self, available_anims, base_assets_available, is_loading_new, secondary_id, primary_id, secondary_assets, interpolation, native_fps, settings);
        self.is_pointer_over_controls = controls_hovered || expand_button_response.hovered();

        crate::features::animation::export::show_popup(ui, &mut self.export_state, self.held_model.as_ref(), self.current_anim.as_ref(), self.held_sheet.as_ref(), &mut self.is_selecting_export_region, settings, available_anims, drag_guard);
    }
}
